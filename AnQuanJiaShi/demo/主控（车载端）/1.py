...
# 行为    双手离开方向盘，使用手机，闭眼，低头，未系安全带，吸烟，打哈欠，酒驾
global_XingWei = ['both_hands_leaving_wheel', 'cellphone', 'eyes_closed', 'head_lowered', 'not_buckling_up', 'smoke', 'yawning', 'jiu_jia']
global_XingWei_ZH = ["双手离开方向盘", "使用手机", "闭眼", "低头", "未系安全带", "吸烟", "打哈欠", "酒驾"]
...
def ShangChuan_ShiShi_MySQL(ZhangHao, ShiJian, WeiZhi, TiWen, XinLv, XueYang):
    global global_Pool
    global global_ShiShiBiao
    sql_shishi_biao = "update %s set ShiJian=(%s), WeiZhi=(%s), TiWen=(%s), XinLv=(%s), XueYang=(%s) where ZhangHao=(%s);" % \
                      (global_ShiShiBiao, ShiJian, WeiZhi, TiWen, XinLv, XueYang, ZhangHao)
    try:
        conn_shishi_biao = global_Pool.connection()  # 从连接池中获取一个连接
        cursor_shishi_biao = conn_shishi_biao.cursor()
        cursor_shishi_biao.execute(sql_shishi_biao)  # 执行sql语句，也可执行数据库命令，如：show tables
        conn_shishi_biao.close()
    except Exception as e:
        conn_shishi_biao.close()
        print(e)
...
def ShangChuan_WeiGui_MySQL(XingWei_ZH):
    ...
    sql_weigui_biao = "insert into %s values(%s,%s,%s,%s,%s,%s,%s);" % \
                      (global_WeiGuiBiao, str(Id), global_time, global_GPS, XingWei_ZH, str(global_TiWen), str(global_XinLv), str(global_XueYang))
    try:
        conn_weigui_biao = global_Pool.connection()  # 从连接池中获取一个连接
        cursor_weigui_biao = conn_weigui_biao.cursor()
        cursor_weigui_biao.execute(sql_weigui_biao)  # 执行sql语句，也可执行数据库命令，如：show tables
        conn_weigui_biao.close()
    except Exception as e:
        conn_weigui_biao.close()
        print(e)
...
def main2():
    ...
    while True:
        ...
        if GPIO_wPi_5 == 0:  # 检测到酒驾行为
            ShangChuan_WeiGui_MySQL(global_XingWei_ZH[7])  # 上传数据库

            YuYin_BoBao(global_XingWei[7])
